...
public class LastMonthFragment extends LazyFragment {
    private static final String TAG = "LastMonthFragment";
    /**每一页展示多少条数据*/
    ...
    private void getHistoryVoucher(int numOfPage, int currentPage) {
       String firstLastMonth = DateUtils.format(DateUtils.getFirstLastMonth(new Date()),"yyyy-MM-dd");
       String firstCurrentMonth = DateUtils.format(DateUtils.getFirstCurrentMonth(new Date()),"yyyy-MM-dd");
//        Log.e(TAG,firstLastMonth +"上月，当月"+firstCurrentMonth);
        subscription = ApiManager.getDefaut().getHistoryVoucher(
                token,firstLastMonth, firstCurrentMonth,Const.ODYCREATETIME,Const.APP,Const.METHOD)
